...
| 🔮 智能体                     | 💡 功能          | 🎯 价值          |
| -------------------------- | -------------- | -------------- |
| **Prediction Agent**       | 预测用户未来的行为和想法   | 比你更早知道你需要什么    |
...
│   │   └── prediction_agent.py # 🔮 AI 行为预测 Agent
│   ├── utils/
│   │   ├── database.py         # 数据库工具
│   │   └── sync_feature_extractor.py # LLM 特征提取
│   └── knowledge_graph/
│       └── graph.py            # 🔗 知识图谱（实时推理）
├── frontend/
│   ├── src/
...
│   └── package.json
├── tests/
│   └── interactive_conversation_test.py # 交互式对话测试
└── data/
...
- 个人信息追踪（姓名、职业、居住地等）
- 社会关系管理
- **🔮 行为预测**：AI 智能预测用户未来最有可能的行为和想法（Top10）

### 🔗 知识图谱

- **全新 ECharts 可视化**：炫酷动画效果，流畅交互体验
- **特征总览视图**：清晰展示各特征类型的分布和关联
...
MindPeek 引入先进的预测能力，主动预判用户未来的行为和想法：

#### 智能预测机制

- **基于特征的智能推断**：结合用户的历史特征、行为模式进行深度分析
- **多维度预测**：涵盖行为、想法、情感、决策等多个方面
- **可能性评分**：每个预测都附带置信度评估（0-100%）
- **时间范围标注**：短期（1-7 天）、中期（1-4 周）、长期（1-3 月）
- **可观察信号**：提供预测的具体验证指标

#### 自动更新策略

- **智能缓存**：24 小时内优先使用缓存结果
- **特征变化检测**：当用户特征数量变化时自动重新预测
- **静默生成**：用户访问页面时后台自动执行，无需手动操作
- **Top10 展示**：按可能性排序显示最有可能的 10 个预测
...
详细设计文档：[docs/agent\_evolution\_design.md](docs/agent_evolution_design.md)
